    ...
    public class SelfSigned
    {
        public SelfSigned(string subject, int port)
        {
            ...
            //if (cert == null)
            {
                ...
                Save(cert);
                Logger.LogInfo($"EmuTarkov.Server: Generated cert with thumbprint {cert.Thumbprint}");
                ...
                CommandPrompt.ExecuteCommand($"netsh http add sslcert ipport=0.0.0.0:{port} certhash={cert.Thumbprint} appid={{{appid}}} certstorename=My");
            }

            Logger.LogInfo($"EmuTarkov.Server: Loading cert with thumbprint {cert.Thumbprint}");
        }
        ...
        private X509Certificate2 Load(string subject)
        {
            try
            {
                ...
                X509Certificate2Collection collection = userStore.Certificates.Find(X509FindType.FindBySubjectName, subject, false);
